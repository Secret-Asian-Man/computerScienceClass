...
//    {
//        node* previousPos=findPrevious(head,position); //get the previous position
//        node* nextPos=position->next;//get the next position
...
//{
//    node* position=search(head,type);
...
//    node* previousNode=findPrevious(head,position);
...
//        {
//            temp=new node(input); //makes a new node with the user defined input
//            position->next=temp;
//            temp->next=NULL;
//        }
//        else
...
//{
//    node* temp; //declares a new pointer
//    temp=new node; //points to a specific place
...
//    {
//        node* start=head;
//        node* end= walker(head,nodeCount(head)-1);
...
//{
//    node* temp=NULL; //this new node is pointing to null
//    node* sourceWalker=walker(source,nodeCount(source)-1); //is at the end of the link-list
...
//    node* end=walker(head,nodeCount(head)-1); //WARNING: takes advantage of the exception in the walker() function, ignore warning!
